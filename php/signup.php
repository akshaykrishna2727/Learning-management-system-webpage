...
if($_SERVER["REQUEST_METHOD"]=="POST")  
        ...
        if(empty($_POST["$lname"]) and !(preg_match("[a-zA-Z]$",$lname)))
            $lnameErr = "Last Name is Required";
        else 
            $lname = test_input($_POST["lname"]);
        if(empty($_POST["$uname"]) and !(preg_match("[a-zA-Z]$",$uname)))
        ...
        if(empty($_POST["$pword"]) and !(preg_match("/*[a-zA-Z]$/",$pword)))
            $pwordErr = "Password is Required";
        else 
            $pword = test_input($_POST["pword"]);
        if(empty($_POST["$cpword"]) and !(preg_match("/*[a-zA-Z]$/",$cpword)))
